...
def common_settings(args: argparse.Namespace, dotenv_values: Dict[str, str]) -> Dict[str, Any]:
    api_key = resolve_value(args.api_key, "NEW_API_KEY", dotenv_values)
    if not api_key and not args.dry_run:
        raise SystemExit("Missing NEW_API_KEY. Set it in the environment or a .env file.")
    ...
    settings = {
        ...
        "base_url": str(resolve_value(args.base_url, "NEW_API_BASE_URL", dotenv_values, DEFAULT_BASE_URL)).rstrip("/"),
        "timeout": timeout,
